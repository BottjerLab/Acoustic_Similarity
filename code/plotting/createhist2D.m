...
%   vertical directions.
%
%   CREATEHIST2D(X,LAMBDA,NBINS,CUTOFF,COLORMAP) plots the
%   histogram with a given colormap handle.  Otherwise the default is 'hot'.
%
%   Example:
...
% modification: for different plottypes
cmaps = {'hsv','hot','gray','bone','copper','pink','white','flag', ...
         'lines','colorcube','vga','jet','prism','cool','autumn',...
         'spring','winter','summer'};

nc = 256;
if any(strcmp(plottype,cmaps))
    cmfh = str2func(plottype);
    cm = cmfh(nc);
else
    cm = hot(nc);
end
...
histcol = zeros([size(intF), 3]);
for ii = 1:size(intF,1)
    for jj = 1:size(intF,2)
        histcol(ii,jj,:) = cm(intF(ii,jj),:);
    end
end
