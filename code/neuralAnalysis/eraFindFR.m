function [histOut, baseIFR] = eraFindFR( ERAWindow, events, type, binSize, spikes, baseline, params )
%get average FR in relation to event onsets or offsets
...
for kk = 1:nUnits
    ...
    tmpHist = tmpHist(1:end-3);
    %histOut(kk,:) = smoothSignal(tmpHist, 15, 'SD'); %removing smoothing
    %for this measure
    histOut(kk,:) = tmpHist;
