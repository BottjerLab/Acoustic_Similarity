function [silentRegions, soundRegions, spectrum] = findSilenceBaseline(songStruct, region, params, varargin)
...
if isempty(songStruct)
    % look at the file itself to determine sampling rate
    fs = getDRSamplingRate(region);
else
...
if params.plot %&& params.verbose
     ...
     title('Labeled regions are sounds');
     plotAreaMarks(soundRegions,[],false);
     subplot(312);
