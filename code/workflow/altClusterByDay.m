...
params = processArgs(defaultParams, 'warpingCost', 1.4);
birdID = 'Lb277';
...
for ii = 1:numel(ages)
    ...
    % pick up to only the first 3 exemplars from each 
    nEx = 5;
    exemplars = NaN(1, nEx * nTypes);
    for jj = 1:nTypes
        foo = find(takenIdxs == jj, nEx);
        exemplars((nEx*jj-nEx)+(1:numel(foo))) = foo;
    end
    ...
    assignedFile = [clustFolder 'altAssignedClustAge-' num2str(ages(ii)) '-' timeFlag];
