function marks = markRegions(songStruct, regions, params, varargin)
...
if ~isempty(songStruct)
    ...
    [~,fsOriginal] = getClip(regions(1), songStruct);
end
...
while ii <= numel(regions)
    ...
    helpStr = '? for help';
    fullHelpStr = 'y to mark, u to undo, r to replay, j/h to +/-pre roll, k/l to +/-post roll, q/w to speed up/slow down, g to jump, ? for help, ! to abort';
    if helpAsked
        helpStr = fullHelpStr;
    end
